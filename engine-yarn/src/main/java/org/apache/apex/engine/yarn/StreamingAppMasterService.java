...
import com.datatorrent.stram.util.MasterServiceUtil.ClusterAppContextImpl;
import com.datatorrent.stram.util.MasterServiceUtil.ClusterAppStats;
...
public class StreamingAppMasterService extends CompositeService implements StreamingAppMaster
{
  ...
  private ClusterAppContextImpl appContext;
  ...
  protected void serviceInit(Configuration conf) throws Exception
  {
    ...
    this.appContext = new ClusterAppContextImpl(dag.getAttributes(), startTime, stats, appAttemptID);
  ...
  private void execute() throws YarnException, IOException
  {
    ...
    long tokenLifeTime = (long)(dag.getValue(LogicalPlan.TOKEN_REFRESH_ANTICIPATORY_FACTOR) * Math.min(dag.getValue(LogicalPlan.HDFS_TOKEN_LIFE_TIME), dag.getValue(LogicalPlanAttributes.RM_TOKEN_LIFE_TIME)));
    ...
    try (YarnClient clientRMService = StramClientUtils.createYarnClient(conf)) {
      ...
      while (!appDone) {
        ...
        if (UserGroupInformation.isSecurityEnabled() && currentTimeMillis >= expiryTime && hdfsKeyTabFile != null) {
          String applicationId = appAttemptID.getApplicationID();

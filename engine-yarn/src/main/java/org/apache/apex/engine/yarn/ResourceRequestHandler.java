...
public class ResourceRequestHandler
{
    ...
    if (!requestedResources.isEmpty()) {
      for (Map.Entry<ContainerStartRequest, MutablePair<Integer, ContainerRequest>> entry : requestedResources.entrySet()) {
        ...
        if ((loopCounter - entry.getValue().getKey()) > NUMBER_MISSED_HEARTBEATS) {
          ...
          LOG.debug("Request for container {} timed out. Re-requesting container", csr.getContainer());
  ...
  {
    int priority = csr.getContainer().getResourceRequestPriority();
    ...
    capability.setMemory(csr.getContainer().getRequiredMemoryMB());
    capability.setVirtualCores(csr.getContainer().getRequiredVCores());
  ...
  {
    String host = null;
    PTContainer c = csr.getContainer();
    if (first) {

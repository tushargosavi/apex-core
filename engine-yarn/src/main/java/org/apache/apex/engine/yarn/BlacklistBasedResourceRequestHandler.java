...
public class BlacklistBasedResourceRequestHandler extends ResourceRequestHandler
{
  ...
  private void recreateContainerRequest(Map<ContainerStartRequest, MutablePair<Integer, ContainerRequest>> requestedResources, int loopCounter, ResourceRequestHandler resourceRequestor, List<ContainerRequest> removedContainerRequests)
  {
    for (Map.Entry<ContainerStartRequest, MutablePair<Integer, ContainerRequest>> entry : requestedResources.entrySet()) {
      if ((loopCounter - entry.getValue().getKey()) > NUMBER_MISSED_HEARTBEATS) {
        ContainerStartRequest csr = entry.getKey();
  ...
  @Override
  public void addContainerRequest(Map<ContainerStartRequest, MutablePair<Integer, ContainerRequest>> requestedResources, int loopCounter, List<ContainerRequest> containerRequests, ContainerStartRequest csr, ContainerRequest cr)
  {
    if (cr.getNodes() != null && !cr.getNodes().isEmpty()) {
  ...
  private void addHostSpecificRequest(ContainerStartRequest csr, ContainerRequest cr)
  {

...
public class StramWSFilterInitializer extends FilterInitializer
{
  ...
  */
  ...
  {
    String addr = conf.get(Settings.Strings.PROXY_ADDRESS.getValue());
    logger.info("proxy address setting {}", addr);
    logger.debug("proxy setting sources {}", conf.getPropertySources(Settings.Strings.PROXY_ADDRESS.getValue()));

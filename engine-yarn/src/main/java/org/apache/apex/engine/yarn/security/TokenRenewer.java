...
public class TokenRenewer
{
  ...
  public TokenRenewer(Context context, boolean renewRMToken, Configuration conf, String destinationFile) throws IOException
  {
    ...
    if (renewRMToken) {
      tokenLifeTime = (long)(context.getValue(LogicalPlan.TOKEN_REFRESH_ANTICIPATORY_FACTOR) * Math.min(context.getValue(LogicalPlan.HDFS_TOKEN_LIFE_TIME), context.getValue(LogicalPlanAttributes.RM_TOKEN_LIFE_TIME)));
      tokenRenewalInterval = (long)(context.getValue(LogicalPlan.TOKEN_REFRESH_ANTICIPATORY_FACTOR) * Math.min(context.getValue(LogicalPlan.HDFS_TOKEN_RENEWAL_INTERVAL), context.getValue(LogicalPlanAttributes.RM_TOKEN_RENEWAL_INTERVAL)));
    ...
    try (FileSystem fs = FileSystem.newInstance(conf)) {
      ...
      if (!checkOnly) {
        try {
          ...
          currUGI.doAs((PrivilegedExceptionAction<Object>)() -> {

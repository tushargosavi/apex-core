...
public class AffinityRulesTest
{
  ...
  public void testOperatorPartitionsAntiAffinity()
  {
    ...
    for (ContainerStartRequest csr : scm.getContainerStartRequests()) {
      ...
      if (csr.getContainer().getOperators().get(0).getName().equals("O2")) {
        Assert.assertNotNull("Host name should not be null", host);
        LOG.info("Partition {} for operator O2 has host = {} ", csr.getContainer().getId(), host);
  ...
  public void testAntiAffinityInOperators()
  {
    ...
    for (ContainerStartRequest csr : scm.getContainerStartRequests()) {
      String host = rr.getHost(csr, true);
      csr.getContainer().host = host;
      if (csr.getContainer().getOperators().get(0).getName().equals("O1")) {

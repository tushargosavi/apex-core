...
public class WindowGeneratorTest
{
  ...
  private static int MAX_WINDOW_ID = 1024;
  ...
  public void test2ndResetWindow() throws InterruptedException
  {
    ...
    WindowGenerator generator = new WindowGenerator(msse, (MAX_WINDOW_ID << 1) + 1024);

    generator.setFirstWindow(0L);
    generator.setResetWindow(0L);
    generator.setWindowWidth(1);

    SweepableReservoir reservoir = generator.acquireReservoir(Node.OUTPUT, (MAX_WINDOW_ID << 1) + 1024);
    ...
    for (int i = 0; i < MAX_WINDOW_ID - 2; i++) {
      msse.tick(1); /* end window and begin window */
    }
    ...
    Assert.assertEquals("begin windows", MAX_WINDOW_ID + 1 + 1, beginWindowCount.get());
    Assert.assertEquals("end windows", MAX_WINDOW_ID + 1, endWindowCount.get());
  }
  ...
  public void testWindowGen() throws Exception
  {
    ...
    WindowGenerator wg = new WindowGenerator(new ScheduledThreadPoolExecutor(1, "WindowGenerator"), MAX_WINDOW_ID + 1024);
  ...
  public void testWindowIdCompare()
  {
    ...
    for (int windowWidthMillis : new int[]{500, 123}) {
      long window1 = WindowGenerator.getWindowId(first, first, windowWidthMillis);
      long window2 = WindowGenerator.getAheadWindowId(window1, first, windowWidthMillis, ahead);
      Assert.assertEquals(ahead, window2 - window1);
    }
  }

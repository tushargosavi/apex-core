...
public class LogicalPlanSerializer extends JsonSerializer<LogicalPlan>
{
  ...
  private static Collection<OperatorMeta> getTopLevelOperators(LogicalPlan plan, ModuleMeta moduleMeta) {
  ...
  private static Collection<StreamMeta> getTopLevelStreams(LogicalPlan plan, ModuleMeta moduleMeta) {
  ...
  private static ArrayList<Map<String, Object>> getLogicalModulesInfo(LogicalPlan dag, boolean flatten)
  {
  ...
  private static Map<String, Object> fillLogicalModuleDetails(LogicalPlan dag, ModuleMeta moduleMeta, boolean flatten)
  {
    ...
    if (flatten) {
      ...
      for (ModuleMeta meta : moduleMeta.getDag().getAllModules()) {
        modulesArray.add(fillLogicalModuleDetails(dag, meta, true));
      }
    }
